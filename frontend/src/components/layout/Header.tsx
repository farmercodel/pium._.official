...
function Header() {
  ...
  const handleItemClick = (k: keyof typeof navigationMap) => navigationMap[k]?.();
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 shadow-sm">
        <nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between p-2 lg:px-8">
          ...
          <PopoverGroup className="hidden lg:flex lg:gap-x-12">
              ...
              <PopoverPanel transition className="absolute left-1/2 z-10 mt-3 w-screen max-w-md -translate-x-1/2
                  ...
                  {POPUP_NAVIGATION_ITEMS.map((item) => (
                    <div key={item.name} className="group relative flex items-center gap-x-6 rounded-lg p-4 text-sm/6 hover:bg-gray-50">
                      ...
                      <div className="flex-auto">
                        <div onClick={() => handleItemClick(item.navigationText as keyof typeof navigationMap)}
                ...
                <div className="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
                  {POPUP_ACTION_ITEMS.map((item) => (
                    <div key={item.name}
                         onClick={() => handleItemClick(item.navigationText as keyof typeof navigationMap)}
            ...
            <div onClick={() => handleItemClick(MAIN_NAVIGATION_ITEMS[1].navigationText as keyof typeof navigationMap)}
                 className="text-sm/6 font-semibold text-gray-900 cursor-pointer">
              {MAIN_NAVIGATION_ITEMS[1].name}
            </div>
            <div onClick={() => handleItemClick(MAIN_NAVIGATION_ITEMS[2].navigationText as keyof typeof navigationMap)}
                 className="text-sm/6 font-semibold text-gray-900 cursor-pointer">
              {MAIN_NAVIGATION_ITEMS[2].name}
            </div>
            <div onClick={() => handleItemClick(MAIN_NAVIGATION_ITEMS[3].navigationText as keyof typeof navigationMap)}
                 className="text-sm/6 font-semibold text-gray-900 cursor-pointer">
          ...
          <div className="hidden lg:flex lg:flex-1 lg:justify-end items-center gap-3">
            {!authChecked ? null : user ? (
              ...
              <>
                <div onClick={() => handleItemClick(MAIN_NAVIGATION_ITEMS[4].navigationText as keyof typeof navigationMap)}
                ...
                <div onClick={() => handleItemClick(MAIN_NAVIGATION_ITEMS[5].navigationText as keyof typeof navigationMap)}
            ...
            <div className="mt-6 flow-root">
              <div className="-my-6 divide-y divide-gray-500/10">
                <div className="space-y-2 py-6">
                  ...
                  {/* Mobile 하단 Auth */}
                  {authChecked && (
                    <div className="mt-4 -mx-3 flex items-center justify-end gap-3 px-3">
                      {user ? (
                        ...
                        <>
                          <Button variant="custom" className="text-gray-700"
                            onClick={() => { handleItemClick('login'); setMobileMenuOpen(false); }}>
                            Login
                          </Button>
                          <Button onClick={() => { handleItemClick('sign up'); setMobileMenuOpen(false); }}>
                            Sign Up
                          </Button>
                        </>
                      )}
