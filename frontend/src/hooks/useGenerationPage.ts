...
export const useGenerationPage = (ideas?: PromotionIdea[]) => {
    ...
    const handleSelect = async (idea: PromotionIdea) => {
        ...
        if (!("__variantId" in idea) || idea.__variantId === undefined || idea.__variantId === null) {
            setModalTitle("게시 불가");
            setModalMessage("이 문안에는 서버의 variant_id가 없어 게시할 수 없습니다. 다시 생성 후 선택해주세요.");
            setModalOpen(true);
            return;
        }
        ...
        if (!image_keys.length) {
            setModalTitle("게시 불가");
            setModalMessage("업로드된 이미지가 없습니다. 이미지 업로드 후 다시 시도해주세요.");
            setModalOpen(true);
            return;
        }
        ...
        try {
            ...
            // 정상 성공
            setModalTitle("게시 완료");
            setModalMessage("인스타그램 게시가 완료되었습니다!");
            setModalOpen(true);
        } catch (error: unknown) {
